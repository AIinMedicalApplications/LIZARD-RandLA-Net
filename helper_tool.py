# from open3d import linux as open3d #Thesis project
...
class ConfigSemantic3D:
    k_n = 16  # KNN
    num_layers = 5  # Number of layers
    num_points = 130000  # Number of input points
    # num_classes = 8  # Number of valid classes #Thesis project
    num_classes = 2  # Number of valid classes
    sub_grid_size = 0.00  # preprocess_parameter #test cases:0.03 0.06 0.09

    batch_size = 2  #4 batch_size during training
    val_batch_size = 16  # batch_size during validation and test
    train_steps = 150#150--for 47 data #500  # Number of steps per epochs
    val_steps = 10 #100  # Number of validation steps per epoch

    sub_sampling_ratio = [4, 4, 4, 4, 2]  # sampling ratio of random sampling at each layer
    d_out = [16, 64, 128, 256, 512]  # feature dimension

    noise_init = 3.5  # noise initial parameter
    max_epoch = 150 #100  # maximum epoch during training
    
    loss_func = "CE+IOU" #  Thesis project: Choose from options--> [(a) cross_entropy, (b) IOU, (c) dice, (d) CE+IOU | [Note: mention class weights after sampling for cross entropy loss]
    learning_rate = 1e-2  # initial learning rate
    lr_decays = {i: 0.95 for i in range(0, 500)}  # decay rate of learning rate
...
class DataProcessing:
    ...
    def get_class_weights(dataset_name):
        ...
        if dataset_name is 'S3DIS':
            ...
            #                          dtype=np.int32) #Thesis project
            num_per_class = np.array([2378207, 319459],
                                      dtype=np.int32)
