...
class Network:
    def __init__(self, dataset, config):
        ...
        with tf.variable_scope('inputs'):
            ...
            self.loss_func  = config.loss_func  #  Thesis project: Different loss functions were tested
        ...
        while self.training_epoch < self.config.max_epoch:
            ...
            try:
                ...
                if m_iou > np.max(self.mIou_list):
                    ...
                    self.saver.save(self.sess, snapshot_directory + '/snap'+str(self.training_epoch), global_step=self.training_step)
    ...
    def get_loss(self, logits, labels, pre_cal_weights):
        if self.loss_func == "cross_entropy":
            class_weights = tf.cast(pre_cal_weights, dtype=tf.float32)
            ...
            # Calculate the IOU loss and define this loss in the helper_tool.py file

            y_true = tf.one_hot(labels, depth=self.config.num_classes)
            y_pred = tf.nn.softmax(logits)

            # Separate the logits and predicted_probs for each class
            y_pred_class_0, y_pred_class_1 = tf.split(y_pred, num_or_size_splits=2, axis=1)
            y_true_class_0, y_true_class_1 = tf.split(y_true, num_or_size_splits=2, axis=1)

            # Calculate IoU for each class
            intersection_class_0 = tf.reduce_sum(y_true_class_0 * y_pred_class_0)
            union_class_0 = tf.reduce_sum(y_true_class_0) + tf.reduce_sum(y_pred_class_0) - intersection_class_0
            iou_class_0 = (intersection_class_0 + .0001) / (union_class_0 + .0001)

            intersection_class_1 = tf.reduce_sum(y_true_class_1 * y_pred_class_1)
            union_class_1 = tf.reduce_sum(y_true_class_1) + tf.reduce_sum(y_pred_class_1) - intersection_class_1
            iou_class_1 = (intersection_class_1 + .0001) / (union_class_1 + .0001)

            output_loss = 1.- (iou_class_1 + iou_class_0)/2.
            ...
            # Calculate intersextion and union for class=1 [as it is a binary class problem finding the loss on any class will be fine]
            intersection_class_0 = tf.reduce_sum(y_true_class_0 * y_pred_class_0)
            union_class_0 = tf.reduce_sum(y_true_class_0) + tf.reduce_sum(y_pred_class_0) - intersection_class_0
            dice_0 = (2. * intersection_class_0 + .0001) / (union_class_0 + intersection_class_0 + .0001)
            
            intersection_class_1 = tf.reduce_sum(y_true_class_1 * y_pred_class_1)
            union_class_1 = tf.reduce_sum(y_true_class_1) + tf.reduce_sum(y_pred_class_1) - intersection_class_1
            dice_1 = (2. * intersection_class_1 + .0001) / (union_class_1 + intersection_class_1 + .0001)
            
            output_loss = 1.- (dice_0 + dice_1)/2.

        elif self.loss_func == "CE+IOU": #composite loss
            #CE
            class_weights = tf.cast(pre_cal_weights, dtype=tf.float32)
            one_hot_labels = tf.one_hot(labels, depth=self.config.num_classes)
            weights = tf.reduce_sum(class_weights * one_hot_labels, axis=1)
            ...
            union_class_1 = tf.reduce_sum(y_true_class_1) + tf.reduce_sum(y_pred_class_1) - intersection_class_1
            iou_class_1 = (intersection_class_1 + .0001) / (union_class_1 + .0001)
            IOU_loss = 1.- (iou_class_1 + iou_class_0)/2.
            
            output_loss = CE_loss + IOU_loss
