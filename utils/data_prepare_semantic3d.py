...
grid_size = 0.00#Thesis project: initially -->0.00, 0.03, 0.06, 0.09
# dataset_path = '/data/semantic3d/original_data' #Thesis project
dataset_path = '/workspace/Storage_fast/point_cloud_data/data_randLA-Net/original_data'
...
for pc_path in glob.glob(join(dataset_path, '*.txt')):
    ...
    if exists(label_path):
        ...
        #                                                           pc[:, 4:7].astype(np.uint8), labels, 0.01) #Thesis project
        sub_points, sub_colors, sub_labels = DP.grid_sub_sampling(pc[:, :3].astype(np.float32),
        ...
        #Thesis project: A small channge--> sending the whole  point cloud to the network
        if grid_size > 0:
            sub_xyz, sub_colors, sub_labels = DP.grid_sub_sampling(sub_points, sub_colors, sub_labels, grid_size)
        else:
            sub_xyz, sub_colors, sub_labels = sub_points, sub_colors, sub_labels
        ...
        #           ['x', 'y', 'z', 'red', 'green', 'blue']) #Thesis project
        ...
        #                                            grid_size=grid_size) #Thesis project
        ...
        if grid_size > 0:
            ...
            sub_xyz, sub_colors = pc[:, :3].astype(np.float32), pc[:, 3:6].astype(np.uint8)
